...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
    ...
    if (f->isLandmarksEnabled()) {
       ...
       int y1[4];
        ...
        int w = x1[1]-x1[0];
        int h = y1[3]-y1[2];
        cv::circle(img, cv::Point(center_x, center_y), 1 + static_cast<int>(0.012 * f->_location.width), cv::Scalar(255, 0, 0), -1);
        ...
        for(int y = center_y-h/8;y < center_y+h/8;y++)
        {
            for(int x = center_x-w/8;x < center_x+w/8;x++)
            {
                ...
                {
                    distance_sum += depth.at<uint16_t>(y,x);
                    effective_pixel++;
                }
            }
        }
		...
		float real_x =(center_x - cam_info.K.at( 2 ) ) / cam_info.K.at( 0 ) * real_z;
		float real_y =(center_y - cam_info.K.at( 5 ) ) / cam_info.K.at( 4 ) * real_z;
		fprintf( stderr, "(%0.2f,%0.2f,%0.2f)\n",real_x, real_y, real_z );
    }

    photoFrameVisualizer->draw(img, f->_location, cv::Scalar(0, 255, 255));
}
